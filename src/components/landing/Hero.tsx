...
function ChromeLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 48" fill="none" className={className}>
      ...
      <path d="M24 4c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12z" 
            fill="none" stroke="url(#chrome-grad)" strokeWidth="2.5"/>
      <defs>
...
// Firefox official logo
function FirefoxLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 48" className={className}>
      <circle cx="24" cy="24" r="20" fill="#FF7139"/>
      <circle cx="24" cy="24" r="10" fill="#FF7139" opacity="0.6"/>
      <circle cx="24" cy="24" r="5" fill="#FF7139" opacity="0.3"/>
      <path d="M24 8 L16 24 L24 22 L32 24 Z" fill="white" opacity="0.9"/>
      <path d="M24 22 L14 34 L24 30 L34 34 Z" fill="white" opacity="0.7"/>
    </svg>
  );
}

// Safari official logo
...
function EdgeLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 48" fill="none" className={className}>
      ...
      <circle cx="24" cy="16" r="2.5" fill="#0078D4"/>
      <circle cx="24" cy="24" r="2.5" fill="#0078D4"/>
      <circle cx="24" cy="32" r="2.5" fill="#0078D4"/>
    </svg>
  );
}
...
function BraveLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 48" fill="none" className={className}>
      <circle cx="24" cy="24" r="20" fill="none" stroke="currentColor" strokeWidth="2.5"/>
      <path d="M24 8 L16 26 L24 22 L32 26 Z" fill="#FB542B"/>
      <path d="M24 22 L12 38 L24 32 L36 38 Z" fill="#FB542B"/>
...
export function Hero() {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-white via-blue-50/30 to-white dark:from-gray-950 dark:via-gray-900/50 dark:to-gray-950">
      {/* Background decoration */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-gradient-to-br from-blue-400/10 to-cyan-400/10 rounded-full blur-3xl animate-pulse-slow" />
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-radial from-transparent via-transparent to-gray-100/50 dark:to-gray-900/50" />
      </div>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6 pt-20 sm:pt-32 pb-32">
        ...
        >
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/30 dark:to-cyan-900/30 border border-blue-200/50 dark:border-blue-700/30 text-sm font-medium text-blue-700 dark:text-blue-300 mb-8"
          >
            <span className="flex h-2 w-2 rounded-full bg-blue-500 animate-pulse" />
            Now with enhanced privacy protection
          </motion.div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-16">
            <Link
              to="/tool"
              className="btn-cta-primary flex items-center justify-center gap-2 text-base sm:text-lg px-6 sm:px-8 py-4 sm:py-5 min-h-[56px] w-full sm:w-auto"
            >
            ...
            <a
              ...
              className="btn-cta-secondary flex items-center justify-center gap-2 text-base sm:text-lg px-6 sm:px-8 py-4 sm:py-5 min-h-[56px] w-full sm:w-auto"
            >
          ...
          >
            {browsers.map(({ name, Logo, color, accent }, index) => (
              <motion.div
                ...
                style={{
                  '--tw-shadow-color': `${color}30`,
                  boxShadow: `0 4px 6px -1px ${color}15`
                } as React.CSSProperties}
              >
                <div 
                  className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${accent}`}
                />
      ...
      <motion.div
        ...
        className="absolute bottom-1/4 left-1/4 w-3 h-3 bg-amber-400 rounded-full opacity-60"
      />
    </section>
  );
}
